...
<div class="max-w-xl mx-auto">
    ...
    <div class="bg-white border border-slate-200 rounded-xl p-6">
        <h2 class="text-lg font-bold text-slate-900 mb-5">Tambah Pendapatan</h2>
            ...
            <div class="mb-6">
                ...
                <textarea id="note" name="note"
                          ...
                          class="w-full px-3.5 py-2.5 text-sm border rounded-lg resize-none focus:outline-none focus:ring-2 transition-colors
                                 {{ $errors->has('note') ? 'border-red-400 bg-red-50 focus:ring-red-300' : 'border-slate-300 focus:border-emerald-500 focus:ring-emerald-100' }}">{{ old('note') }}</textarea>
                @error('note')
                <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
