...
<div class="max-w-xl mx-auto">
    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
            <div>
                <p class="text-xs text-slate-500 dark:text-slate-400 mb-1">Pengeluaran</p>
                <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ rupiah($expense->amount) }}</p>
            </div>
        ...
        <dl class="divide-y divide-slate-100 dark:divide-slate-700">
            ...
            <div class="px-6 py-4 flex items-center justify-between">
                ...
                <dd class="text-sm font-medium text-slate-700 dark:text-slate-300 text-right max-w-xs">{{ $expense->description }}</dd>
            </div>

            <div class="px-6 py-4 flex items-center justify-between">
                <dt class="text-sm text-slate-500 dark:text-slate-400">Kategori</dt>
                <dd class="text-sm font-medium text-slate-700 dark:text-slate-300">{{ $expense->category->name }}</dd>
            </div>
            ...
            <div class="px-6 py-4 flex items-center justify-between">
                <dt class="text-sm text-slate-500 dark:text-slate-400">Terakhir diubah</dt>
                <dd class="text-sm text-slate-500 dark:text-slate-400">
        ...
        <div class="mx-6 my-4 flex items-start gap-3 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg px-4 py-3">
            ...
            <div>
                <p class="text-xs font-semibold text-slate-700 dark:text-slate-200">Belum diverifikasi</p>
        ...
        <div class="mx-6 my-4 flex items-start gap-3 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg px-4 py-3">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                 ...
                 class="w-5 h-5 text-emerald-500 dark:text-emerald-400 flex-shrink-0">
                <path d="M20 13c0 5-3.5 7.5-8 9-4.5-1.5-8-4-8-9V5l8-3 8 3z"/>
                <path d="m9 12 2 2 4-4"/>
            </svg>
            <div>
                <p class="text-xs font-semibold text-emerald-700 dark:text-emerald-300">Terverifikasi</p>
        ...
        <div class="mx-6 my-4 flex items-start gap-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg px-4 py-3">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                 fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
                <p class="text-xs font-semibold text-amber-700 dark:text-amber-300">Perlu Perhatian</p>
        ...
        <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-700 flex gap-3">
            ...
            @if(in_array($expense->verified_status, ['pending', 'verified', 'flagged']))
                @php $verification = $expense->verification; @endphp
                @if($verification)
    ...
    <div class="mt-4 text-center">
        <a href="{{ route('expenses.index') }}" class="text-sm text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">
